...
export class KeyRing {
  ...
  public async createMnemonicKey(
    ...
    // const ethWallet = new ethers.Wallet(hexlify(privKey.toBytes()));
    // this.misesService.initQueryClient();
    this.misesService.activateUser(hexlify(privKey.toBytes()));
  ...
  public async unlock(password: string) {
    ...
    const privKey = this.loadPrivKey(60);
    // const ethWallet = new ethers.Wallet(privKey.toBytes());
    // this.misesService.initQueryClient();
    this.misesService.activateUser(hexlify(privKey.toBytes()));
  }
  ...
  private loadKey(coinType: number, useEthereumAddress: boolean = false): Key {
    ...
    if (useEthereumAddress) {
      ...
      const wallet = new ethers.Wallet(hexlify(privKey.toBytes()));
      return {
        algo: "ethsecp256k1",
  ...
  public async signEthereum(
    ...
    switch (type) {
      ...
      case EthSignType.EIP712: {
        ...
        const signature = await new ethers.SigningKey(
          hexlify(privKey.toBytes())
        ).sign(
          Hash.keccak256(
            Buffer.concat([
              // eth separator
